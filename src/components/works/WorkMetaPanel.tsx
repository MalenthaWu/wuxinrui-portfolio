  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-2xl bg-surface p-4 ring-1 ring-black/[0.04]">
        ...
        <div className="rounded-2xl bg-surface p-4 ring-1 ring-black/[0.04]">
          ...
          <ul className="mt-2 space-y-1">
            {work.decisions.map((item) => (
              <li key={item} className="text-[0.8125rem] text-muted">
                · {item}
              </li>
            ))}
          </ul>
      ...
      <div className="rounded-2xl bg-accent/5 p-4 ring-1 ring-accent/10">
      ...
      {work.externalUrl && (
        <a
          ...
          className="inline-flex rounded-full bg-accent px-5 py-2.5 text-[0.8125rem] font-medium text-white transition-opacity hover:opacity-90"
        >
          查看完整作品集 PDF →
        </a>
      )}
